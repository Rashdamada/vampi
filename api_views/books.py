...
from flask import jsonify, Response, request, json, redirect
...
def get_book_blurb(book):
    ...
    if "expired" in resp:
        ...
        try:
            with open('./text/' + book, 'r') as f:
                return Response(json.dumps(f.read()), 200, mimetype="application/json")
        except:
            return Response(error_message_helper("Book not found!"), 404, mimetype="application/json")

def get_book_redirect1(url):
    resp = token_validator(request.headers.get('Authorization'))
    if "expired" in resp:
        return Response(error_message_helper(resp), 401, mimetype="application/json")
    elif "Invalid token" in resp:
...
def get_book_redirect2(url):
    ...
    if "expired" in resp:
        ...
        try:
            return Response(headers={'Refresh':f'0; url={url}'})
        except:
            return Response(error_message_helper("Book not found!"), 404, mimetype="application/json")